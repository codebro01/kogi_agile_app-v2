...
export const ViewPaymentsRecords = () => {
  ...
  const [mergingStudents, setMergingStudents] = useState(false)
  const [mergedStudentsReady, setMergedStudentsReady] = useState(false)
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/payments/view-payments`, {
        ...
        params: { ...filteredParams, download: true },
        withCredentials: true,
      })
      const { getAllPaymentsRecords } = response.data
      setFetchAttendanceLoading(false)
      exportToCSV(getAllPaymentsRecords, 'Students Payment Record')
  ...
  const mergeStudentsDataIntoPayments = async () => {
    setMessage('Merging Students Payments, Please wait .........')
    setMergingStudents(true)
    try {
      const token = localStorage.getItem('token')

      console.log('entered the mergeStudentsDataIntoPayments')
      const response = await axios.patch(
        `${API_URL}/payments/merge-students-data-into-payments`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setMessage(message)
      setTimeout(() => setMergingStudents(''), 10000)
      setMergingStudents(false)
      setMergedStudentsReady(true)
      console.log(message)
    } catch (err) {
      setMergedStudentsReady(false)

      setMergingStudents(false)
      setMessage(
        err?.response?.message ||
          err?.response?.data?.message ||
          'Something went wrong while merging students payments, please try again...'
      )
      setTimeout(() => setMergingStudents(''), 10000)
  ...
  const columns = [
...

    {
      name: 'School Name',
      selector: (row) => {
        const foundSchool = uniqueSchools.find(
          (school) => school.schoolId === row.schoolId
        )
        return foundSchool ? foundSchool.schoolName : 'Unknown'
      },
      sortable: true,
    },
    // {
    ...
    <>
      ...
      userPermissions.includes('handle_payments') ? (
        <Container maxWidth="lg" sx={{ marginTop: 4, marginBottom: '50px' }}>
          ...
          >
            <Button
              onClick={mergeStudentsDataIntoPayments}
              variant="contained"
              disabled={mergingStudents}
              sx={{
                display: 'flex',
                gap: 4,
              }}
            ...
            <Box>
              ...
              >
                ...
                <Grid container spacing={2} alignItems="center">
                  ...
                  <Grid item xs={12} sm={6} md={4}>
                    <InputLabel id="paymentType-label" sx={{ marginBottom: 1 }}>
                      Payment Type
                    </InputLabel>
                    <Select
                      name="paymentType"
              ...
              >
                ...
                >
                  <Typography
                    variant="h4"
                    sx={{ fontWeight: 700, color: '#196b57' }}
                  >
                ...
                >
                  ...
                  >
                    ...
                    <p>
                      ...
                      {selectedItem.studentRandomId}
                    </p>
                    <p>
                      <strong>Name:</strong>{' '}
                      {`${selectedItem.surname} ${selectedItem.firstname} ${selectedItem.middlename || ''
                        }`}
                    </p>
                    <p>
                      <strong>School name:</strong> {selectedItem.schoolName}
                    </p>
                    <p>
                    ...
                    <p>
                      <strong>Cummulated Attendance Score:</strong>{' '}
                      {selectedItem.totalAttendanceScore}
                    </p>
                    <p>
                      <strong>Month:</strong> {selectedItem.month}
                    </p>
                    <p>
