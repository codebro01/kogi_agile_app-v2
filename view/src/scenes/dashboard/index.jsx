...
import {
  StudentsContext,
  SchoolsContext,
} from '../../components/dataContext.jsx'
...
const Dashboard = () => {
      ...
      const fetchData = async () => {
        try {
          const res = await axios.get(
            `${API_URL}/verifier/verifier-dashboard`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  if (dashboardData) {
    ...
    distinctSchools =
      dashboardData?.results?.[0]?.distinctSchools[0]?.totalDistinctSchools
    total_primary_6 = dashboardData?.results?.[0]?.studentsByClass?.find(
      (classData) => classData.className === 'Primary 6'
    )
    total_primary_6 = total_primary_6?.totalStudentsInClass
    total_JSS_3 = dashboardData?.results?.[0]?.studentsByClass?.find(
      (classData) => classData.className === 'JSS 3'
    )
    total_JSS_3 = total_JSS_3?.totalStudentsInClass

    total_JSS_1 = dashboardData?.results?.[0]?.studentsByClass?.find(
      (classData) => classData.className === 'JSS 1'
    )
    total_JSS_1 = total_JSS_1?.totalStudentsInClass

    total_SSS_1 = dashboardData?.results?.[0]?.studentsByClass?.find(
      (classData) => classData.className === 'SSS 1'
    )
    ...
    totalScienceAndVocational =
      dashboardData?.schoolCategory?.[0]?.totalScienceAndVocational
  }
  ...
  if (userPermissions.includes('handle_payments')) {
    ...
    return (
      <>
        {lgaWithTotalPayments && totalStudentsPaid && (
          <Box
            sx={{
              padding: {
                xs: '10px',
          ...
          >
            <Header
              sx={{
                ...
                color: colors.dashboardStatBox['darkGreen'],
              }}
              title={`${greetingMessage} ${capitalize(
          ...
          >
            ...
            >
              ...
              >
                <StatBox
                  title={formatWithCommas(
                    verifierDashboardData[0]?.totalStudents
                  )}
                  subtitle="Total Number of Students Enrolled"
                  progress="0.75"
                  borderRadius="5px"
              ...
              >
                <StatBox
                  title={`${formatWithCommas(
                    verifierDashboardData[0]?.verifiedCount
                  )}`}
                  subtitle="Total Verified Students"
                  progress="0.75"
                  borderRadius="5px"
                  icon={
                    <EmailIcon
              ...
              >
                <StatBox
                  title={`${formatWithCommas(
                    verifierDashboardData[0]?.unverifiedCount
                  )}`}
                  subtitle="Total Unverified Students"
                  progress="0.75"
                  borderRadius="5px"
                  icon={
                    <EmailIcon
              ...
              >
                <StatBox
                  title={`${formatWithCommas(
                    verifierDashboardData[0]?.schoolsWithVerifiedStudentsCount
                  )}`}
                  subtitle="Total Schools with verified Students"
                  progress="0.75"
                  borderRadius="5px"
                  icon={
                    <EmailIcon
          ...
          >
            ...
            >
              <CardContent>
                ...
                >
                  Here, you can efficiently manage and oversee student data with
                  ease. The platform is designed to simplify your tasks and
                  provide you with the tools needed for effective data
                  management.
                </Typography>
              </CardContent>
            </Card>
            ...
            >
              <Typography
                ...
                sx={{
                  color: colors.main.darkGreen,
                  background:
                    'linear-gradient(135deg, rgba(224, 224, 224, 1), rgba(200, 200, 200, 1))',
                  fontWeight: 'bold',
                  letterSpacing: '1px',
                  textAlign: 'center',
                  padding: '20px 15px',
              ...
              >
                <Table sx={{ minWidth: 650 }}>
                  ...
                  <TableBody>
                    {dashboardData?.recentTwentyStudents?.map(
                      (student, index) => (
                        <TableRow
                          key={index}
                          sx={{
