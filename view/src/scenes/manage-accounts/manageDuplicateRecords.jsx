...
export const ManageDuplicateRecords = () => {
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      const response = await axios.delete(
        `${API_URL}/student/delete/delete-many/?ids=${ids}`,
        {
          headers: {
        ...
        .map((student) => {
          return {
            ...
            similarRecords: student.similarRecords.filter(
              (similarStudent) =>
                !selectedStudents.includes(similarStudent.randomId)
            ),
          }
        })
  ...
  const handleDelete = async (id, surname, firstname) => {
    try {
      const confirmDelete = window.confirm(
        `Are you sure you want to delete ${surname} ${firstname} `
      )
      if (!confirmDelete) return
      setDeleteLoading(true)
      ...
      const updatedStudents = students
        .map((student) => {
          return {
            ...student,
            similarRecords: student.similarRecords.filter(
              (similarStudent) => similarStudent.randomId !== id
            ),
          }
        })
        .filter((student) => student.similarRecords.length > 0) // Remove students with no similarRecords
  ...
  const fetchDuplicate = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/student/manage-duplicate-records`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
      ...
      >
        ...
        {fetchDataLoading ? (
          ...
          <Box sx={{ overflowX: 'scroll', maxHeight: '700px', width: '100%' }}>
            {students.length > 0 ? (
              <Table
                sx={{ overflowX: 'scroll', maxHeight: '700px' }}
                  ...
                  {students.map((student, index) =>
                    student?.similarRecords?.map((similarStudent, subIndex) => (
                      <TableRow key={similarStudent._id}>
                        <TableCell>
                          <Checkbox
                            checked={selectedStudents.includes(
                              similarStudent.randomId
                            )}
                            onChange={() =>
                              handleCheckboxChange(similarStudent.randomId)
                            }
                            sx={{
                              // '& .MuiSvgIcon-root': {
                        ...
                        {/* Nested data check */}
                        <TableCell>
                          {similarStudent.lgaOfEnrollment || 'N/A'}
                        </TableCell>
                        <TableCell>
                          {similarStudent.presentClass || 'N/A'}
                        </TableCell>
                        <TableCell>
                          {similarStudent.parentPhone || 'N/A'}
                        </TableCell>
                        <TableCell
                          onClick={() =>
                            handleDelete(
                              similarStudent.randomId,
                              similarStudent.surname,
                              similarStudent.firstname
                            )
                          }
