...
export const ExportSchoolAttendance = () => {
  ...
  const percentageOption = [{ name: '70% and Above', value: '70' }]
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/student/view-attendance-sheet`,
        {
          headers: {
      ...
      if (error.response.status === 404)
        return setApiResp(
          'No record for filter, please try filtering something else!'
        )
      setApiResp(
        error.response?.message ||
          error.response.data.message ||
          'File proccessin failed Please try again'
      )
      setTimeout(() => setApiResp(''), 6000)
    }
  }
      ...
      <Grid container spacing={4} sx={{ width: '100%' }}>
        ...
        <Grid item xs={12} sm={6}>
          <Autocomplete
            ...
            value={
              uniqueSchools.find(
                (school) => school.schoolId === filters.schoolId
              ) || null
        ...
        <Grid item xs={12} sm={6}>
          ...
          >
            ...
            <MenuItem value={getCurrentYear()}>{getCurrentYear()}</MenuItem>
          </Select>
        </Grid>
        ...
        <Grid item xs={12} sm={6}>
          <Select
            ...
            onChange={(e) => handleSelectChange(e, { name: 'session' })}
            displayEmpty
            fullWidth
            size="medium"
          ...
          >
            ...
            {['2024/2025', '2025/2026', '2026/2027'].map((session, index) => (
        ...
        <Grid item xs={12} sm={6}>
          <Select
            name="term"
            value={filters.term}
            onChange={(e) => handleSelectChange(e, { name: 'term' })}
            displayEmpty
            fullWidth
            size="medium"
            labelId="term-label"
        ...
        {/* Select on those with account Number */}
          ...
          <FormControlLabel
            control={<Checkbox 
                ...
                onChange = {(e)=> setFilters(prev => {
                    ...
                    return {
                      ...prev,
                      withBankDetails: checked,
                    }
                })}
                />}
            label="Select only students with bank details"
          />
        </FormGroup>
      </Box>
