...
export const getSingleSchool = async (req, res, next) => {
  try {
    ...
    if (!school)
      return next(new NotFoundError(`There is no school with id: ${id}`))
...
export const createSchool = async (req, res, next) => {
  try {
    const isExistingSchool = await AllSchools.findOne({
      schoolCode: req.body.schoolCode,
    })
    if (isExistingSchool)
      return next(new BadRequestError('School already exist'))

    const newSchool = await AllSchools.create({ ...req.body })

    if (!newSchool)
      return next(new BadRequestError('An error occured adding School'))
...
export const updateSchool = async (req, res, next) => {
  try {
    ...
    const updateSchool = await AllSchools.findOneAndUpdate(
      { _id: id },
      { ...req.body },
...
export const deleteSchool = async (req, res, next) => {
  try {
    ...
    res
      ...
      .json({ id: id, message: `School successfully deleted` })
  } catch (err) {
    return next(err)
  }
}
