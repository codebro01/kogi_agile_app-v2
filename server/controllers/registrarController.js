...
export const getSingleRegistrar = async (req, res, next) => {
  try {
    const { id } = req.params
    const registrar = await Registrar.findById({ _id: id }).select(
...
export const createRegistrar = async (req, res, next) => {
  try {
    ...
    const isExistingAccountNumber = await Registrar.findOne({
...
export const updateRegistrar = async (req, res, next) => {
  try {
    ...
    if (!registrar)
      return next(new NotFoundError('There is no registrar with id: ' + id))

    const updatedRegistrar = await Registrar.findOneAndUpdate(
      { _id: id },
      { ...req.body },
      { new: true, runValidators: true }
    )
    if (!updatedRegistrar)
      return next(new Error('An Error occured while trying to update registra'))
...
export const deleteRegistrar = async (req, res, next) => {
  try {
    ...
    res
      .status(StatusCodes.OK)
      .json({ message: `Useer hase been successfully deleted` })
  } catch (err) {
    return next(err)
  }
}
