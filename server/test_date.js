...
mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/KOGI_AGILE_DB_TEST')
  .then(async () => {
    ...
    if (fromDate || toDate) {
      ...
      if (fromDate) attendanceMatchQuery.date[''] = new Date(fromDate);
      if (toDate) attendanceMatchQuery.date[''] = new Date(new Date(toDate).setHours(23, 59, 59, 999));
    }
    ...
    // Also check what is in the DB for that school around that time:
    const anyDate = await SchoolAttendance.findOne({ schoolId: new mongoose.Types.ObjectId('676f4cb12b490484a5fa4172') }).sort({ date: -1 }).select('date').lean();
    console.log('Most recent date in DB for this school:', anyDate?.date);
