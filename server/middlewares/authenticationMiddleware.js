...
export const authMiddleware = async (req, res, next) => {
  ...
  try {
    ...
    if (registrar) {
      const permissionsArray = registrar.roles.flatMap((role) =>
        role.permissions.map((permission) => permission.name)
      )
    ...
    if (payrollSpecialist) {
      const permissionsArray = payrollSpecialist.roles.flatMap((role) =>
        role.permissions.map((permission) => permission.name)
      )
  ...
  return (req, res, next) => {
    ...
    if (!user || !user.permissions || user.permissions.length === 0) {
      return res
        ...
        .json({ message: 'User does not have any permissions' })
    }
    ...
    if (!hasPermission) {
      return res
        .status(StatusCodes.FORBIDDEN)
        .json({ message: 'User does not have the required permissions' })
    }
